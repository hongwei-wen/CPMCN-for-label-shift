...
def read_preds(fh):
    return np.array([[float(x) for x in y.rstrip().split("\t")] for y in fh])
...
def run_calibmethods(valid_preacts, valid_labels, test_preacts, test_labels,
                     calibname_to_calibfactory, samplesize, samplesizesseen,
    ...
    for calibname, calibfactory in\
        ...
        calibfunc = calibfactory(valid_preacts=valid_preacts,
                                 valid_labels=valid_labels)

        unshifted_test_preds = calibfunc(test_preacts)
        unshifted_test_nll = -np.mean(
            np.sum(np.log(unshifted_test_preds) * test_labels, axis=-1))
        unshifted_test_ece = abstention.calibration.compute_ece(
            softmax_out=unshifted_test_preds, labels=test_labels, bins=15)
        ...
        if (samplesize not in samplesizesseen):
            metric_to_samplesize_to_calibname_to_unshiftedvals['ece'][
                samplesize][calibname].append(unshifted_test_ece)
            metric_to_samplesize_to_calibname_to_unshiftedvals['nll'][
                samplesize][calibname].append(unshifted_test_nll)
            metric_to_samplesize_to_calibname_to_unshiftedvals['jsdiv'][
                samplesize][calibname].append(unshifted_test_jsdiv)
...
def run_experiments(num_trials, seeds, alphas_and_samplesize, shifttype,
                    calibname_to_calibfactory,
                    imbalanceadaptername_to_imbalanceadapter,
                    adaptncalib_pairs, validglobprefix, testglobprefix,
                    valid_labels, test_labels):

    draw_test_indices = get_func_to_draw_label_proportions(test_labels)

    alpha_to_samplesize_to_adaptername_to_metric_to_vals = (defaultdict(
        lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(list)))))
    alpha_to_samplesize_to_baselineacc = defaultdict(lambda: defaultdict(list))
    ...
    for (alpha, samplesize) in alphas_and_samplesize:
        for seed in seeds:
            ...
            for trial_num in range(num_trials):
                ...
                sample_valid_indices = rng.choice(a=np.arange(
                    len(valid_preacts)),
                                                  size=samplesize,
                                                  replace=False)
                ...
                (calibname_to_calibfunc,
                 calibname_to_calibvalidpreds) = (run_calibmethods(
                ...
                for metricname in metric_to_samplesize_to_calibname_to_unshiftedvals:
                    calibname_to_unshiftedvals = metric_to_samplesize_to_calibname_to_unshiftedvals[
                        metricname][samplesize]
                    best_calibname = min(
                        list(calibname_to_unshiftedvals.keys()),
                        key=lambda x: calibname_to_unshiftedvals[x][-1])
                    calibname_to_calibfunc[
                        'best-' +
                        metricname] = calibname_to_calibfunc[best_calibname]
                    calibname_to_calibvalidpreds[
                        'best-' + metricname] = calibname_to_calibvalidpreds[
                            best_calibname]

                # NEWLY CHANGED for cifar100
                if (shifttype == 'dirichlet'):
                    altered_class_priors = rng.dirichlet(np.ones(100) * alpha)
                elif (shifttype == 'tweakone'):
                    rd_idxes = rng.permutation(100)[:10]
                    altered_class_priors = np.full(100, (1.0 - alpha) / 90.0)
                    altered_class_priors[rd_idxes] = alpha / 10.0
                else:
                ...
                shifted_test_baseline_accuracy = np.mean(
                    ...
                    np.argmax(abstention.calibration.softmax(
                        preact=shifted_test_preacts, temp=1.0, biases=None),
                              axis=-1))
                ...
                print("GT:  {}".format(ideal_shift_weights))
                ...
                for adapter_name, calib_name in adaptncalib_pairs:
                    ...
                    print(adapter_name, calib_name, shift_weights)
                    unnormed_estimshiftedpriors = np.mean(
                        sample_valid_labels, axis=0) * shift_weights
                    estim_shifted_priors = (
                        unnormed_estimshiftedpriors /
                    ...
                    adapted_shifted_test_accuracy = np.mean(
                        np.argmax(shifted_test_labels, axis=-1) == np.argmax(
                            adapted_shifted_test_preds, axis=-1))
                    ...
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                        alpha][samplesize][
                            adapter_name + ":" +
                            calib_name]['mseweights_testsetprop'].append(
                                np.sum(true_shifted_priors *
                                       (np.square(ideal_shift_weights -
                                                  shift_weights))))
                    #mse weights but each class weighted evenly
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                        alpha][samplesize][
                            adapter_name + ":" +
                            calib_name]['mseweights_even'].append(
                                np.mean(
                                    np.square(ideal_shift_weights -
                                              shift_weights)))
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                        alpha][samplesize][
                            adapter_name + ":" + calib_name]['jsdiv'].append(
                                scipy.spatial.distance.jensenshannon(
                                    p=true_shifted_priors,
                                    q=estim_shifted_priors))
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                        alpha][samplesize][adapter_name + ":" +
                                           calib_name]['delta_acc'].append(
                                               delta_from_baseline)
        ...
        for metric_name in [
                'delta_acc', 'jsdiv', 'mseweights_testsetprop',
                'mseweights_even'
        ]:
            ...
            for adapter_name, calib_name in adaptncalib_pairs:
                ...
                n = len(
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[alpha]
                    [samplesize][adaptncalib_name][metric_name])
                print(
                    adaptncalib_name, "MEDIAN",
                    np.median(
                        alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                ...
                print(
                    ...
                    "+/-", (1.0 / np.sqrt(n)) * np.std(
                        alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                            alpha][samplesize][adaptncalib_name][metric_name],
                        ddof=1))
                if metric_name == "mseweights_even":
                    print(
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[alpha]
                    [samplesize][adaptncalib_name][metric_name])
                sys.stdout.flush()
