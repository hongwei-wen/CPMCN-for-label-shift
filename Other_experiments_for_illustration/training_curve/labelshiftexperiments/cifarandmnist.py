    ...
    for (alpha,samplesize) in alphas_and_samplesize:
            ...
            for trial_num in range(num_trials):  
                ...
                for adapter_name,calib_name in adaptncalib_pairs:
                    ...
                    imbalance_adapter_func = imbalance_adapter(
                        ...
                        ideal_shift_weights=ideal_shift_weights)
                    shift_weights = imbalance_adapter_func.multipliers
                    print(adapter_name, calib_name, shift_weights)
                    print("MSE:",np.mean(np.square(ideal_shift_weights-shift_weights)))
                    ...
                    print("ACC:",adapted_shifted_test_accuracy)
                    ...
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                        alpha][samplesize][adapter_name+":"+calib_name][
                        'acc_base'].append(shifted_test_baseline_accuracy)
                    alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                        alpha][samplesize][adapter_name+":"+calib_name][
                        'acc_adapted'].append(adapted_shifted_test_accuracy)
        ...
        for metric_name in ['delta_acc', 'jsdiv',
            ...
            for adapter_name,calib_name in adaptncalib_pairs:
                ...
                print(adaptncalib_name, "MEDIAN", np.median(
                 alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                   alpha][samplesize][adaptncalib_name][metric_name]), "+/-",
                 ...
                 alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                   alpha][samplesize][adaptncalib_name][metric_name],
                   ddof=1))
                print(adaptncalib_name, "MEAN", np.mean(
                 alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                   alpha][samplesize][adaptncalib_name][metric_name]), "+/-",
                 (1.0/np.sqrt(n))*np.std(
                 alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                   alpha][samplesize][adaptncalib_name][metric_name],
                   ddof=1))
                print(alpha_to_samplesize_to_adaptername_to_metric_to_vals[
                   alpha][samplesize][adaptncalib_name][metric_name])
                sys.stdout.flush()
